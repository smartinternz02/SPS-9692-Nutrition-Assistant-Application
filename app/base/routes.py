...
def food_item():
    ...
    temp = re.findall(r'\d+', plaintext)
    res = list(map(int, temp))
    print("total calories ", res[1])
    calories = res[1]

    food = Food(title=message, content=plaintext, eaten=current_user, calories=calories)
...
def food_view():
    ...
    foods = Food.query.filter_by(eaten=user).order_by(Food.date.desc()).paginate(page=page, per_page=5)
